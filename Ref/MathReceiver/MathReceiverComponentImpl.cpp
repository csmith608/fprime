...
namespace Ref {
  ...
  //the port invoked by the scheduler retrieves the messages from the message queue
  //and dispatches them.
  //The message dispatches invoke the command and input port handlers that were implemented earlier
  void MathReceiverComponentImpl ::
    SchedIn_handler(
        const NATIVE_INT_TYPE portNum,
    ...
    )
  {
    ...
    //factor1s is the number of times factor1 has been written
